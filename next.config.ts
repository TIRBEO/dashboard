    ...
    return [
      {
        ...
        headers: [
          ...
          { key: 'Content-Security-Policy', value: `default-src 'self'; script-src 'self' 'unsafe-inline' ${isDev ? "'unsafe-eval'" : ""}; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https://api.tirbeo.app; font-src 'self' https://fonts.gstatic.com; connect-src 'self' ${isDev ? 'http://localhost:3000 http://127.0.0.1:3000' : ''} https://api.tirbeo.app; form-action 'self'; frame-ancestors 'none'; base-uri 'self'; object-src 'none';` },
        ],
      },
    ];
  },
