...
export default function DeleteAccountPage() {
    ...
    return (
      <div className="p-6 lg:p-8 max-w-lg mx-auto text-center">
        <div className="p-8 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)]">
          ...
          <button onClick={() => router.push('/dashboard')} className="px-4 py-2.5 rounded-lg bg-[var(--color-primary)] text-white text-sm font-medium">Return to dashboard</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-lg mx-auto">
      ...
      <div className="p-8 rounded-xl bg-[var(--color-surface)] border border-[var(--color-error)]/20">
        ...
        <input type="text" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="DELETE"
          className="w-full p-3 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-error)] transition-colors mb-4" />
